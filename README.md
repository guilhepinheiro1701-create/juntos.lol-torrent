...
`SS_WORKER_STORAGE_DIRS` (opcional) é onde o worker aceita guardar filmes, como uma lista de `rótulo=caminho`: `SSD=/mnt/ssd/juntos,HDD=/mnt/hdd/juntos`. Vazio, o único lugar é `SS_WORKER_DATA_DIR/torrents`, que é como sempre foi. Com dois ou mais, a página mostra um seletor em Baixados e manda o rótulo junto do torrent; o worker resolve o rótulo do lado dele e recusa um que não conheça. O que trafega é sempre o rótulo, nunca um caminho — aceitar caminho vindo do navegador seria escrita em qualquer lugar do disco para quem abrisse uma sala. Os caminhos precisam ser absolutos e são criados na subida: um que não possa ser criado derruba o worker, porque descobrir isso no meio de um download é descobrir tarde demais.
