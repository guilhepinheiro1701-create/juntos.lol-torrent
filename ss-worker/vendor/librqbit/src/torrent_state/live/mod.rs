...
impl PeerHandler {
    ...
    fn acquire_next_piece(&self) -> crate::Result<Option<(ValidPieceIndex, bool)>> {
        if self.is_choked() {
        ...
        let result = self
            ...
            .with_live_mut(self.addr, "acquire_next_piece", |live| {
                ...
                match result {
                    ...
                    AcquireResult::Joined(piece) => {
                        debug!("joined co-download of piece {}", piece);
                        Ok(Some((piece, true)))
                    }
    ...
    async fn task_peer_chunk_requester(&self) -> crate::Result<()> {
        ...
        loop {
            ...
            let (next, joined) = match self.acquire_next_piece()? {
                Some(next) => next,
                None => {
            ...
            // ss patch: a joiner fetches the chunks the owner has not
            // downloaded yet, back to front, so co-downloaders meet in the
            // middle instead of duplicating each other's requests.
            let mut chunks: Vec<_> = if joined {
                let missing = self
                    .state
                    .lock_read("joined_missing_chunks")
                    .get_chunks()?
                    .missing_chunks(next);
                let mut v: Vec<_> = self
                    ...
                    .filter(|c| missing.get(c.chunk_index as usize).map(|b| *b).unwrap_or(false))
                    .collect();
                v.reverse();
                v
            } else {
                self.state.lengths.iter_chunk_infos(next).collect()
            };
        ...
        fn write_to_disk(
            ...
            let ppl_guard = {
                ...
                match g.get_pieces()?.get_inflight(chunk_info.piece_index) {
                    Some(inflight) if inflight.peers.contains(&addr) => {}
                    Some(inflight) => {
                        debug!(
                            "in-flight piece {} was stolen by {:?}, ignoring",
                            chunk_info.piece_index, inflight.peers
                        );
                        return Ok(());
                    }
                    None => {
